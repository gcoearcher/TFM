...
# Reducir dimensiones con t-SNE (parámetros optimizados)
print("\nAplicando t-SNE...")
tsne_reducer = TSNE(
    n_components=2,
    perplexity=30,        # Controla el balance entre estructura local y global
    learning_rate=200,    # Velocidad de optimización
    n_iter=1000,         # Número de iteraciones
    early_exaggeration=12, # Factor de exageración temprana
    random_state=42,
    metric='cosine',      # Usar distancia coseno como en UMAP
    init='pca',          # Inicialización con PCA para mayor estabilidad
    verbose=1            # Mostrar progreso
)
tsne_2d = tsne_reducer.fit_transform(embeddings)

# Añadir coordenadas t-SNE al dataframe filtrado
df_filtered = df_filtered.copy()  # Evitar warnings
df_filtered["x"] = tsne_2d[:, 0]
df_filtered["y"] = tsne_2d[:, 1]

# Aplicar K-Means clustering para forzar 4 clusters bien definidos
kmeans = KMeans(n_clusters=4, random_state=42, n_init=10)
cluster_labels = kmeans.fit_predict(tsne_2d)
...
ax1.set_title("Clustering por Emoción Original (t-SNE)", fontsize=14, fontweight='bold')
...
ax2.set_title("Clustering por K-Means con t-SNE (4 clusters forzados)", fontsize=14, fontweight='bold')
...
# Opcional: Guardar las coordenadas t-SNE y clusters para análisis posteriores
df_filtered.to_csv("emociones_con_clusters_tsne.csv", index=False)
print(f"\nArchivo con coordenadas t-SNE y clusters guardado como 'emociones_con_clusters_tsne.csv'")
print(f"Se analizaron {len(df_filtered)} textos en total con 4 clusters forzados usando t-SNE")