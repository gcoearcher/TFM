...
print(f"Las 3 emociones principales son: {top_emotions}")
...
embeddings = model.encode(df_filtered["Comment"].tolist(), show_progress_bar=True)
...
print("\nExtrayendo palabras clave con KeyBERT...")
...
for cluster_id in np.unique(hdbscan_labels):
    if cluster_id == -1:  # Ruido
        continue
    cluster_docs = [df_filtered["Comment"].iloc[i] for i in range(len(hdbscan_labels)) if hdbscan_labels[i] == cluster_id]
...
if n_clusters_hdb > 0:
    for i in range(n_clusters_hdb):
        ...
        keywords = ", ".join(cluster_keywords.get(i, [])[:5])  # solo top 5
...
# Análisis de pureza de clusters (comparación con etiquetas originales)
print("\n📈 Análisis de pureza de clusters:")
for i in range(n_clusters_hdb):
    cluster_data = df_filtered[df_filtered["hdbscan_cluster"] == i]
    emotion_counts = cluster_data["main_emotion"].value_counts()
    dominant_emotion = emotion_counts.index[0]
    purity = emotion_counts.iloc[0] / len(cluster_data)
    print(f"  Cluster {i}: Emoción dominante '{dominant_emotion}' ({purity:.2%} pureza)")

print(f"\n✅ Análisis completado usando MiniLM para {len(df_filtered)} textos")