...
for i in range(n_clusters):
    ...
    results.append({
        "Cluster": i,
        "Textos": len(cluster_data),
        "Emociones": emotion_dist,
        "Keywords": ", ".join(keywords[:5])
    })

st.subheader("Resultados por cluster")
st.dataframe(pd.DataFrame(results))

# ------------------------
# 6. Visualización scatter
# ------------------------
...
for i in range(n_clusters):
    ...
    ax.scatter(umap_2d[cluster_mask, 0], umap_2d[cluster_mask, 1],
    ...
    ax.text(center_x, center_y, top_keywords, fontsize=10, weight='bold',
            ha='center', va='center',
            bbox=dict(facecolor='white', alpha=0.7, edgecolor='gray', pad=3))
...
ax.set_title("Clustering de Emociones con KeyBERT + MiniLM", fontsize=16, pad=20)
